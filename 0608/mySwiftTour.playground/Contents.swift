...
// 7.adjust() // 會噴錯誤，swift 視它為常數
var aInt = 99
99.simpleDescription
aInt.adjust()
...
//print(protocolValue.anotherProperty) 由於Comfort Protocol所以不能看到anotherProperty屬性
...
do{
    let printResponse1 = try send(job: 1234, toPrinter: "Never Has Toner")
    print(printResponse1)
}catch{
    print(error)
...
do {
    let printerResponse = try send(job: 1440, toPrinter: "No Paper")
    print(printerResponse)
...
do {
    let printerResponse = try send(job: 1440000, toPrinter: "On Fire")
    print(printerResponse)
} catch PrinterError.onFire {
    print("I'll just put this over here, with the rest of the fire.")
} catch let printerError as PrinterError {
...
makeArray1(repeating: 456, numberOfTimes: 4)
//<Item> Item為placeholder
...
makeArray(repeating: 123, numberOfTimes: 4)

//任意型別~Any,  AnyObject(限Class Instance)
...
let myArray3 = makeArray3(repeating: "rrr", numberOfTimes: 4)  //陣列裡面是Any Type
(myArray3[1] as!  String) + "DEf"  // 使用需轉型as!



// Reimplement the Swift standard library's optional type
enum OptionalValue<Wrapp> {
    case none
    case some(Wrapp)
}
...
func anyCommonElements<T: Sequence, U: Sequence>(_ lhs: T, _ rhs: U) -> Array<T.Iterator.Element>
    where T.Iterator.Element: Equatable, T.Iterator.Element == U.Iterator.Element {
        ...
        var array234 = Array<T.Iterator.Element>()
        for lhsItem in lhs {
            for rhsItem in rhs {
                if lhsItem == rhsItem {
                    array234.append(lhsItem)
                    //return true
                }
            }
        }
...
func anyCommonElements3<L, R>(_ lhs:[L], _ rhs:[R]) -> Array<L>{
    var myArray = Array<L>()
    for lhsItem in lhs{
        for rhsItem in rhs{
            if lhsItem == rhsItem
